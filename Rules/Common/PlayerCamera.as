...
void onRender(CRules@ this)
{
	...
	if (localPlayer is null || getLocalPlayerBlob() !is null) return;
	...
	if (localPlayer.getTeamNum() == this.getSpectatorTeamNum())
	{
		drawHelpText();
	}
}
...
void drawHelpText()
{
	const int time = getGameTime();

	GUI::SetFont("menu");

	//const int endTime1 = helptime + (getTicksASecond() * 5);
	const int endTime2 = helptime + (getTicksASecond() * 3);
	...
	if (time < endTime2)
	{
		text = "If you click on a ship the camera will follow it.";
	}
	...
	if (text != "")
	{
		...
		lr = ul + size;
		//wiggle up and down
		f32 wave = Maths::Sin(time / 10.0f) * 5.0f;
		ul.y += wave;
		lr.y += wave;
		//draw
