...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	if (isServer() && other_color > 0 && color != other_color) // block vs block
	{
		...
		const bool docking = (this.hasTag("coupling") || blob.hasTag("coupling")) && this.getTeamNum() == blob.getTeamNum()
				&& ((ship.isMothership || other_ship.isMothership) || (ship.isSecondaryCore || other_ship.isSecondaryCore) || ship.isBuildStation)
		...
		else
		{
			...
			else if (!this.hasTag("removable"))
			{
				...
				if (this.hasTag("ramengine"))
				{
					...
					else if (blob.hasTag("propeller") || blob.hasTag("engineblock") || blob.hasTag("vulnerable"))
				...
				{
					if (blob.hasTag("propeller") || blob.hasTag("plank") || blob.hasTag("engineblock") || blob.hasTag("vulnerable"))
					{
